...
export async function POST(req: Request) {
    try {
        ...
        if (!fileData) {
            return Response.json({ error: 'File not found based on the fileId' }, { status: 404 });
        }
    
        const updatedVectors = await updatePptxFile(fileData.id, file);
        const updatedFile: KnowledgeFile = { ...fileData, vectorIds: updatedVectors, updatedAt: new Date().toISOString() };
